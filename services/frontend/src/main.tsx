...
// pelo host em `window.__FRONTEND_CONFIG__` no embed do Frontend.
...
declare global {
  interface Window {
    __FRONTEND_CONFIG__?: FrontendConfig;
  }
}
...
const embed = window.__FRONTEND_CONFIG__;
...
if (!token && !isBypass) {
  ...
  const config: FrontendConfig = embed
    ? { ...embed, token }
    : {
